...
/// BIFF FTAB lookup — returns (function name, arg count) for common
/// built-ins. `arg count` is needed for PtgFunc (fixed arity). For
/// PtgFuncVar we still call this to get the name but ignore the
/// arg count. Indexed by iftab from the formula's PtgFunc/PtgFuncVar
/// token. Not exhaustive — returns None for entries we haven't
/// catalogued, causing decoders to bail rather than guess.
fn ftab_lookup(iftab: u16) -> Option<(&'static str, usize)> {
    // Subset of the BIFF FTAB table covering what the gop templates
    // actually use. For fixed-arity entries the argc is non-zero;
    // for variable-arity entries the argc here is 0 (only the
    // PtgFuncVar caller uses the stored argc anyway).
    Some(match iftab {
        0 => ("COUNT", 0),          // variable
        1 => ("IF", 0),             // variable (2-3 args)
        2 => ("ISNA", 1),
        3 => ("ISERROR", 1),
        4 => ("SUM", 0),            // variable
        5 => ("AVERAGE", 0),
        ...
        247 => ("DB", 0),
        252 => ("FREQUENCY", 2),
        255 => return None,       // User — handled separately
        261 => ("ERROR.TYPE", 1),
        269 => ("AVEDEV", 0),
        271 => ("GAMMALN", 1),
        273 => ("CRITBINOM", 3),

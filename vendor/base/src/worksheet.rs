...
impl Worksheet {
    ...
    /// Toggle the `hidden` flag on a row, creating a default Row entry
    /// if none exists yet (rows are sparse — only stored when they have
    /// non-default attributes). Mirrors set_row_height's "find or create"
    /// pattern.

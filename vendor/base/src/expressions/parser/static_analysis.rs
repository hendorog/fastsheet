...
fn get_function_args_signature(kind: &Function, arg_count: usize) -> Vec<Signature> {
    match kind {
        ...
        // fastsheet UDFs — every arg stays a Vector so we get raw ranges
        // (no implicit-intersection per-cell collapse), and the FILTER mask
        // can be a comparison-of-ranges that we walk ourselves.
        Function::MyUnique => vec![Signature::Vector; arg_count],
        Function::MySort => vec![Signature::Vector; arg_count],
        Function::MySortBlank => vec![Signature::Vector; arg_count],
        Function::MyTranspose => vec![Signature::Vector; arg_count],
        Function::MyFilter => vec![Signature::Vector; arg_count],
    }
}
...
fn static_analysis_on_function(kind: &Function, args: &[Node]) -> StaticResult {
    match kind {
        ...
        // fastsheet additions
        Function::Char => StaticResult::Scalar,
        // fastsheet UDFs — each call returns a single cell from the
        // would-be spill, so the static result is Scalar.
        Function::MyUnique => StaticResult::Scalar,
        Function::MySort => StaticResult::Scalar,
        Function::MySortBlank => StaticResult::Scalar,
        Function::MyTranspose => StaticResult::Scalar,
        Function::MyFilter => StaticResult::Scalar,
    }
}

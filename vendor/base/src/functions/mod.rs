...
pub enum Function {
    ...
    // fastsheet additions — Excel functions IronCalc 0.7.1 doesn't ship.
    Char,

    // fastsheet additions — pseudo-spill UDFs ported from gop's POI workbook.
    // Each takes (data..., anchor) where the anchor is the top-left cell of the
    // intended spill range. The function uses (cell - anchor) offset to pick
    // its single-cell return from the would-be spill matrix. Because gop's
    // xlsx wraps each in `<f t="array" ref="X:Y">`, the load preprocessor
    // expands the CSE array formula into per-cell formulas so every cell in
    // the spill range evaluates the function with its own (row, col).
    MyUnique,
    MySort,
    MySortBlank,
    MyTranspose,
    ...
    ($($field:ident => $variant:ident),+ $(,)?) => {
        impl Functions {
            pub fn lookup(&self, name: &str) -> Option<Function> {
                ...
                // fastsheet pseudo-spill UDFs + missing-from-IronCalc
                // built-ins — names are not localized.
                match key.as_str() {
                    "CHAR" => return Some(Function::Char),
                    "MYUNIQUE" => return Some(Function::MyUnique),
                    "MYSORT" => return Some(Function::MySort),
                    "MYSORTBLANK" => return Some(Function::MySortBlank),
                    "MYTRANSPOSE" => return Some(Function::MyTranspose),
                    "MYFILTER" => return Some(Function::MyFilter),
                    _ => {}
                }
                $(
                    if self.$field == key {
...
impl Function {
    pub fn to_localized_name(&self, language: &Language) -> String {
        ...
        match self {
            ...
            // fastsheet additions — non-localized literals.
            Function::Char => "CHAR".to_string(),
            Function::MyUnique => "MYUNIQUE".to_string(),
            Function::MySort => "MYSORT".to_string(),
            Function::MySortBlank => "MYSORTBLANK".to_string(),
            Function::MyTranspose => "MYTRANSPOSE".to_string(),
            Function::MyFilter => "MYFILTER".to_string(),
        }
    }
...
impl<'a> Model<'a> {
    pub(crate) fn evaluate_function(
        ...
        match kind {
            ...
            Function::MyUnique => self.fn_my_unique(args, cell),
            Function::MySort => self.fn_my_sort(args, cell),
            Function::MySortBlank => self.fn_my_sort_blank(args, cell),
            Function::MyTranspose => self.fn_my_transpose(args, cell),
            Function::MyFilter => self.fn_my_filter(args, cell),
        }
    }
}

    ...
    pub(crate) fn evaluate_cell(&mut self, cell_reference: CellReferenceIndex) -> CalcResult {
        ...
        match cell.get_formula() {
            Some(f) => {
                ...
                // Normalise the return value through the just-stored
                // cell so callers see the SAME type on first evaluation
                // and on cached subsequent evaluations. Without this,
                // raw evaluator outputs that set_cell_value rewrites
                // (Range / Array → NIMPL error; EmptyCell → stored as
                // CellFormulaNumber{v:0.0}) leak to the caller on the
                // first run and produce different behaviour from the
                // cached run. The bug bites IF-returns-empty + LEN
                // pairs (LEN(C30) at H30 saw EmptyCell on first run
                // → length 0; second run would have returned Number 0
                // → length 1, matching Excel) and IFERROR(HLOOKUP(...))
                // where HLOOKUP's target cell had a multi-cell-range
                // formula.
                let row_data = &self.workbook.worksheets[cell_reference.sheet as usize]
                    .sheet_data[&cell_reference.row];
                let cell = &row_data[&cell_reference.column];
